...
// The issue is the JSX structure is fundamentally broken.
// Let's just extract the header (imports + types + logic) and replace the entire JSX return
...
const headerCode = c.substring(0, retIdx);
// Find the last closing brace of the function
const endIdx = c.lastIndexOf("}");
if (endIdx === -1) {
  console.error("Cannot find closing brace");
  process.exit(1);
}

// We'll write a completely new return block
// The issue is that the JSX from the write-notifs.mjs script got duplicated

// Let's just rebuild the entire file from scratch
// Build clean JSX
...
import { CheckCheck, Trash2, Clock, RefreshCw } from "lucide-react";
...
export default function NotificationsPage() {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      if (loading) toast.error("Failed to load notifications");
    } finally {
      setLoading(false);
    }
  }, [filter, loading]);
  ...
  const groupedNotifs = filter === "all" ? groupNotifications(notifications) : [["Unread" as TimeGroup, notifications.filter((n) => !n.read)]];
                  ...
                  return (
                    ...
                    >
                      <Card
                        ...
                        className={cn(!notification.read && "bg-indigo/[0.02]")}
                      >
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1 min-w-0 space-y-2.5">
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                {!notification.read && (
                                  <span className="inline-flex h-2 w-2 rounded-full bg-indigo shadow-[0_0_0_3px_rgba(79,70,229,0.15)]" />
                                )}
                              </div>
                              <p
                                className={
                                  notification.read
                                    ? "text-sm sm:text-base leading-relaxed text-muted-foreground"
                                    : "text-sm sm:text-base leading-relaxed text-foreground font-medium"
                                }
                              >
...
fs.writeFileSync(p, cleanFile, "utf-8");
console.log("Written clean file. Length:", cleanFile.length);

// Verify
const opens = (cleanFile.match(/<div[^>]*>/g) || []).length;
const closes = (cleanFile.match(/<\/div>/g) || []).length;
const cardOpens = (cleanFile.match(/<Card[^>]*>/g) || []).length;
const cardCloses = (cleanFile.match(/<\/Card>/g) || []).length;
const motionOpens = (cleanFile.match(/<motion\.div[^>]*>/g) || []).length;
const motionCloses = (cleanFile.match(/<\/motion\.div>/g) || []).length;

console.log("div opens:", opens, "closes:", closes);
console.log("Card opens:", cardOpens, "closes:", cardCloses);
console.log("motion.div opens:", motionOpens, "closes:", motionCloses);

if (opens === closes && cardOpens === cardCloses && motionOpens === motionCloses) {
  console.log("ALL TAGS BALANCED!");
} else {
  console.log("TAGS MISMATCHED");
  console.log("Missing divs:", opens - closes);
  console.log("Missing Cards:", cardOpens - cardCloses);
  console.log("Missing motion.divs:", motionOpens - motionCloses);
}
